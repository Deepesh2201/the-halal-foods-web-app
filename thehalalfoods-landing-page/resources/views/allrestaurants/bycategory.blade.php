...
async function getStores(id) {
    ...
    var storesRef = database.collection('vendors').where("reststatus", "==", true);
        ...
        if(alldata.length){
	        ...
	        alldata.forEach((listval) => {
			 ...
			 if(hour<10){hour='0'+hour} if(minute<10){minute='0'+minute}
	            ...
	            html = html +'<div class="member-plan position-absolute"><span class="badge badge-dark '+statusclass+'">'+status+'</span></div><a href="'+view_vendor_details+'"><img alt="#" src="'+photo+'" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body"><h6 class="mb-1"><a href="'+view_vendor_details+'" class="text-black">'+val.title+'</a></h6><h6>'+val.location+'</h6>';
